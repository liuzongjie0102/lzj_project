...
import com.lzj.test.ReportReqBean;
import com.lzj.test.ReportRspBean;
import com.lzj.test.RorCInfo;
...
//        row_dim_list.add(r5);
        ...
        reqBean.setSubtotalField(null);
        String[][] new_datas = transDataSet(reqBean).getDatas();
    ...
    public static ReportRspBean transDataSet(ReportReqBean bean) throws Exception{
        ...
        int row_offset =  0;
        if(isNeedTitle){
            col_offset =  (!is_target_inCol && target_size > 0) || row_dim_size != 0 ? 1 :0;
            row_offset =  (is_target_inCol && target_size > 0) || col_dim_size != 0 ? 1 :0;
        }
        ...
        int y_col_index = row_dim_size+row_offset;//y轴的行扩展
        int x_row_index = col_dim_size+col_offset;//x轴的列扩展
        ...
        if(row_index + col_dim_size + col_offset > 50){
            throw new Exception("数据区域超过设计范围");
        }
        String[][] newDatas = new String[col_index + row_dim_size + row_offset][row_index + col_dim_size + col_offset];

        //添加表头
        if(isNeedTitle){
            for (int i = 0; i < row_dim_size; i++){
                newDatas[i+row_offset][0] = rowDimList.get(i).getElement_name();
            }
            for (int j = 0; j < col_dim_size; j++){
                newDatas[0][j+col_offset] = colDimList.get(j).getElement_name();
            }

            if (is_target_inCol){
                for (int j = col_dim_size + col_offset; j < row_index + col_dim_size + col_offset - (isNeedTotal?1:0);){
                ...
                for (int j = row_dim_size + row_offset; j < col_index + row_dim_size + row_offset - (isNeedTotal?1:0);){
        ...
        //填写维度的值
        y_col_index = row_dim_size+row_offset;//y轴的行扩展
        x_row_index = col_dim_size+col_offset;//x轴的列扩展
        for (int n = 0, size = colList.size(); n < size; n++) {
            List<String> list = colList.get(n);
            ...
            for (int i = 0; i < list.size(); i++) {
                newDatas[y_col_index][i+col_offset] = list.get(i);
            }
            y_col_index++;
            if(col_subIndex != -1 && n == size -1){
                for (int i = 0; i < list.size(); i++) {
        ...
        for (int n = 0, size = rowList.size(); n < size; n ++) {
            ...
            for (int i = 0; i < list.size(); i++) {
                newDatas[i+row_offset][x_row_index] = list.get(i);
            }
            x_row_index++;
            if(row_subIndex != -1 && n == size -1){
                for (int i = 0; i < list.size(); i++) {
            ...
            if (!targetTempMap.isEmpty()) {
                if (is_target_inCol) {
                    ...
                    if(StringUtils.isEmpty(sb_col)){
                        y = row_dim_size + row_offset;
                    }else{
                        y = colMap.get((sb_col.toString())) + row_dim_size + row_offset;
                    }
                    //小计位置偏移
                    ...
                    for (String key : targetTempMap.keySet()) {
                        ...
                        if(StringUtils.isEmpty(sb_row)){
                            x = col_dim_size + col_offset +(temp_index++);
                        }else{
                            x = rowMap.get(sb_row.toString() + "|" + key) + col_dim_size + col_offset;
                        }
                        //小计位置偏移
                    ...
                    int x = 0;
                    if(StringUtils.isEmpty(sb_row)){
                        x = col_dim_size + col_offset;
                    }else{
                        x = rowMap.get(sb_row.toString()) + col_dim_size + col_offset;
                    }
                    //小计位置偏移
                    for (Integer index : sub_row_index) {
                        if (x >= index){
                    ...
                    for (String key : targetTempMap.keySet()) {
                        int y = 0;
                        if(StringUtils.isEmpty(sb_col)){
                            y = row_dim_size + row_offset +(temp_index++);
                        }else{
                            y = colMap.get((sb_col.toString() + "|" + key)) + row_dim_size + row_offset;
                        }
                        //小计位置偏移
                        for (Integer index : sub_col_index) {
                            if (y >= index){
        ...
        //计算小计的值
        int tempCol = row_dim_size+row_offset;
        for (Integer index : sub_col_index) {
            for (int i = 0; i< row_index; i++){
                int col = col_dim_size+col_offset+i;
                int subSum = 0;
        ...
        int tempRow = col_dim_size+col_offset;
        for (Integer index : sub_row_index) {
            for (int i = 0; i< col_index; i++){
                int row = row_dim_size+row_offset+i;
                int subSum = 0;
        ...
        if(isNeedTotal){
            if(row_subIndex != -1){
                //合计在右方
                for (int i=is_target_inCol?0:row_offset; i < row_dim_size + row_offset; i++){
                    newDatas[i][col_offset+col_dim_size+row_index-1] = "总计";
                }
                int temp = row_dim_size + row_offset;
                for (int i=0; i < col_index; i++){
                    int sum = 0;
                    for (Integer index : sub_row_index) {
                        sum = sum + Integer.parseInt(newDatas[temp][index]);
                    }
                    newDatas[temp++][col_offset+col_dim_size+row_index-1] = sum+"";
                }
            }else if (col_subIndex != -1){
                //合计在下方
                for (int i=is_target_inCol?col_offset:0; i < col_dim_size + col_offset; i++){
                    newDatas[row_offset+row_dim_size+col_index-1][i] = "总计";
                }
                int temp = col_dim_size + col_offset;
                for (int i=0; i < row_index; i++){
                    int sum = 0;
                    for (Integer index : sub_col_index) {
                        sum = sum + Integer.parseInt(newDatas[index][temp]);
                    }
                    newDatas[row_offset+row_dim_size+col_index-1][temp++] = sum+"";
                }
            }
        }
