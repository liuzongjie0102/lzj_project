...
public class CallableTask1 implements Callable<Object> {
    private static BaseLogger log = (BaseLogger) BaseLogger.getLogger(CallableTask1.class);

    PLSqlToolQueryService plSqlToolQueryService =(PLSqlToolQueryService)SpringContextUtils.getBean("com.newland.edc.cct.plsqltool.interactive.service.impl.PLSqlToolQueryServiceImpl");
    PLSqlToolTestService  plSqlToolTestService  =(PLSqlToolTestService)SpringContextUtils.getBean("com.newland.edc.cct.plsqltool.interactive.service.impl.PLSqlToolTestServiceImpl");
    ...
    public CallableTask1(List<TestSqlBean> testSqlBeans){
                ...
                try {
                    ...
                    if(execute_type.equals("DQL")){
                        if(this.testSqlBeans.get(m).getDbType().equals("hive")){
                            ...
                            sql = "SELECT * FROM ("+ this.testSqlBeans.get(m).getReal_sql() +") v_alias_12 limit 1000";
                            ...
                            sql = "SELECT 1 as gp_rownum,* FROM ("+this.testSqlBeans.get(m).getReal_sql()+") t1 limit 1000 offset 0";
                        }
                    }else{
                    ...
                    if(execute_type.equals("DDL")) {
                        ...
                        if(ugi!=null){
                            ugi.doAs( new PrivilegedExceptionAction<Void>(){
                                @Override
                                public Void run() throws Exception{
                                    if(key_col.equals("CREATE_TABLE") || key_col.equals("CREATE_VIEW")
                            ...
                            if(key_col.equalsIgnoreCase("CREATE_TABLE")||key_col.equalsIgnoreCase("CREATE_VIEW")){
                                ...
                                if(!result.getResult_col_list().isEmpty()){
                                    ...
                                    for (int i=0;i<result.getResult_col_list().size();i++){
                                        ...
                                        temporaryColInfo.setCol_chs_name(result.getResult_col_list().get(i).getCol_id());
                                        ...
                                        }else if(this.testSqlBeans.get(m).getDbType().equals("oracle")||this.testSqlBeans.get(m).getDbType().equals("db2")||this.testSqlBeans.get(m).getDbType().equals("greenplum")){
                                            if(result.getResult_col_list().get(i).getCol_length().equals("0")){
                                                temporaryColInfo.setCol_length("");
                                            }else{
                                                temporaryColInfo.setCol_length(result.getResult_col_list().get(i).getCol_length());
                                            }
                                        }
                                        if(result.getResult_col_list().get(i).getCol_precise()!=null&&!result.getResult_col_list().get(i).getCol_precise().equals("")&&Integer.parseInt(result.getResult_col_list().get(i).getCol_precise())>0){
                                    ...
                                    for (int i=0;i<temporaryTabInfos.size();i++){
                                        if(!result.getResult_col_list().isEmpty()){
                                            ...
                                            for (int j=0;j<result.getResult_col_list().size();j++){
                                                ...
                                                temporaryColInfo.setCol_chs_name(result.getResult_col_list().get(j).getCol_id());
                                                ...
                                                if(this.testSqlBeans.get(m).getDbType().equals("hive")){
                                                    if(!result.getResult_col_list().get(j).getCol_type().equalsIgnoreCase("STRING")&&result.getResult_col_list().get(i).getCol_type().toUpperCase().indexOf("INT")==-1){
                                                        ...
                                                        temporaryColInfo.setCol_length("");
                                                                ...
                                                                ||this.testSqlBeans.get(m).getDbType().equals("db2")
                                                                ||this.testSqlBeans.get(m).getDbType().equals("greenplum")){
                                                    if(result.getResult_col_list().get(j).getCol_length().equals("0")){
                                                        temporaryColInfo.setCol_length("");
                                                    }else{
                                                        temporaryColInfo.setCol_length(result.getResult_col_list().get(j).getCol_length());
                                                    }
                        ...
                        if(ugi!=null){
                            colnum = ugi.doAs( new PrivilegedExceptionAction<Integer>(){
                                @Override
                                public Integer run() throws Exception{
                                    rs = ps.executeQuery();
                                    rsmd = rs.getMetaData();
                        ...
                        if(this.testSqlBeans.get(m).getDbType().equals("hive")){
                            if(ugi!=null){
                                DQL_result_col_list = ugi.doAs( new PrivilegedExceptionAction<List<DgwResultCol>>(){
                                    @Override
                                    public List<DgwResultCol> run() throws Exception{
                        ...
                        if(ugi!=null){
                            DQL_result_data_list = ugi.doAs( new PrivilegedExceptionAction<List<DgwResultRowData>>(){
                                @Override
                                public List<DgwResultRowData> run() throws Exception{
    ...
    protected void dealDB2ColumnInfo(TemporaryTabInfo temporaryTabInfo,String table_name,String tabschema) throws Exception{
        ...
        try {
            String parSql = "select DATAPARTITIONEXPRESSION from sysibm.sysdatapartitionexpression where tabname='"+table_name.toUpperCase()+"' and tabschema='"+tabschema.toUpperCase()+"'";
            log.info("db2查询"+table_name+"建表分区，sql："+parSql);
            createTablePs = conn.prepareStatement(parSql);
            createTableRs = createTablePs.executeQuery();
            while(createTableRs.next()){
                if(StringUtils.isNotBlank(createTableRs.getString("DATAPARTITIONEXPRESSION"))){
                    parColumns.add(createTableRs.getString("DATAPARTITIONEXPRESSION"));
                }
            }
        }catch (Exception parExecption){
            log.info("查询分区信息失败");
